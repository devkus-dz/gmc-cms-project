...
/**
 * @interface TagItem
 * @description Structure of a Tag object.
 */
 ...
 * @description Props for the TagsTable component.
 */
interface TagsTableProps {
    data: TagItem[];
    isLoading: boolean;
...
/**
 * @function TagsTable
 * @description Presentation component for the Tags data table using TanStack.
 * @param {TagsTableProps} props - Component properties.
 * @returns {JSX.Element} The rendered table with search and pagination.
 */
export default function TagsTable({ data, isLoading, editingId, onEdit, onDelete }: TagsTableProps) {
    ...
    const columns = useMemo(() => [
        ...
        columnHelper.accessor('description', {
            header: 'Description',
            cell: (info) => (
                <p className="text-slate-500 line-clamp-2 text-sm">{info.getValue() || '—'}</p>
            ),
        }),
        columnHelper.display({
            ...
            cell: (info) => (
                <div className="flex items-center justify-end gap-2">
                    ...
                    <button
                        onClick={() => onDelete(info.row.original.tag_id)}
            ...
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col">
                            ...
                            {isLoading ? (
                                ...
                                <tr>
                                    <td colSpan={columns.length} className="px-6 py-12 text-center text-slate-500">
                                        {globalFilter ? 'No tags found matching your search.' : 'No tags created yet.'}
                                    </td>
                                </tr>
                            ) : (
                                table.getRowModel().rows.map((row) => (
                                    <tr
                                        key={row.id}
                                        className={`transition-colors ${editingId === row.original.tag_id ? 'bg-blue-50' : 'hover:bg-slate-50'}`}
                                    >
                ...
                <div className="px-6 py-4 border-t border-slate-200 bg-slate-50 flex items-center justify-between">
                    <div className="text-sm text-slate-500">
                        Showing <span className="font-medium text-slate-900">{table.getRowModel().rows.length}</span> of <span className="font-medium text-slate-900">{table.getFilteredRowModel().rows.length}</span>
                    </div>
